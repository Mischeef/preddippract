...
# Установим текущую рабочую директорию
os.chdir(r'C:\Users\gerpv\Desktop\VibeTEST')
...
# Выбор batch_size
batch_size = 64  # Начнем с разумного значения
num_epochs = 10  # Увеличьте количество эпох для увеличения общего количества шагов
...
for epoch in range(num_epochs):
    ...
    for i, (inputs, labels) in enumerate(train_loader):
        inputs = [x.permute(1, 0).unsqueeze(0) for x in inputs]  # Преобразование формата данных
        labels = labels.long()
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            inputs = [x.permute(1, 0).unsqueeze(0) for x in inputs]
            labels = labels.long()
            outputs = torch.cat([model(x) for x in inputs])
